"""Protected admin HTTP surface for operational management."""
...
def grant(req: GrantRequest, user: User = Depends(_current_admin), db: Session = Depends(get_db)) -> dict:
    try:
...
def revoke(req: RevokeRequest, user: User = Depends(_current_admin), db: Session = Depends(get_db)) -> dict:
    try:
