...
class CommercialApiContractTests(unittest.TestCase):
    ...
    def test_quota_contract_does_not_leak_internal_user_id(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), {"credits_remaining": 3})
        self.assertNotIn("user_id", response.json())
        self.assertNotIn("id", response.json())

    def test_consume_contract_does_not_leak_entitlement_id(self):
        ...
        self.assertEqual(body["credits_remaining"], 2)
        self.assertNotIn("entitlement_id", body)
        self.assertNotIn("id", body["user"])

    def test_logout_revokes_session(self):
        user = SimpleNamespace(id=15, public_id="public-15", name="Out", role="user", status="active")
        with patch("commercial_api.resolve_session", return_value=user), patch("commercial_api.revoke_session") as revoke:
            response = self.client.post("/api/v1/auth/logout", headers={"Authorization": "Bearer token"})
        self.assertEqual(response.status_code, 200, response.text)
        self.assertEqual(response.json(), {"revoked": True})
        revoke.assert_called_once()

    def test_create_payment_is_server_authoritative_and_omits_internal_ids(self):
        ...
        expected = {
            ...
            "callback_url": "https://example.test/callback?order_id=order-12",
        }
        ...
        self.assertEqual(response.status_code, 200, response.text)
        body = response.json()
        self.assertNotIn("payment_id", body)
        self.assertNotIn("user_id", body)
        self.assertEqual(body["order_id"], "order-12")
        self.assertEqual(body["amount_rial"], 2_490_000)
