...
function startNewJourney() {
  try {
    ...
    state.likedCodes = [];
    if (!state.likedCodesSet || typeof state.likedCodesSet.clear !== 'function') {
    ...
    state.swipeIndex = 0;
    state.totalSwipes = 0;
    if (!state.completedPaths || typeof state.completedPaths.clear !== 'function') state.completedPaths = new Set();
    else state.completedPaths.clear();
    if (!state.completedSubRealms || typeof state.completedSubRealms.clear !== 'function') state.completedSubRealms = new Set();
    else state.completedSubRealms.clear();
    state.lastPayload = null;
    state.cachedMotives = null;
    state.majorsResult = null;
    state.branchesResult = null;

    // دادهٔ قلمروها از data.js باید موجود باشد
    if (typeof REALMS === 'undefined' || !REALMS || !REALMS.length) {
      app.innerHTML = `<div class="card" style="margin:20px;text-align:right;">
        ...
        <p style="color:#888;font-size:.85rem">آدرس را چک کن: data.js در ریشهٔ همان پوشهٔ index باشد.</p>
...
function dhResumeFromSplash() {
  if (window.__dhSavedSession) {
    try {
      ...
      if (['manifesto','guide','splash'].includes(state.stage)) state.stage = 'realm';
    } catch (e) {
      state.stage = 'realm';
    }
  } else {
