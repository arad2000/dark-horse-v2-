...
function ensureUXStyles() {
  ...
  s.id = 'dh-ux-styles';
  s.textContent = `
    .dh-progress-wrap{position:sticky;top:0;z-index:50;background:rgba(10,10,15,.92);backdrop-filter:blur(8px);padding:10px 0 8px;margin-bottom:8px}
    .dh-progress-labels{display:flex;justify-content:space-between;font-size:.7rem;color:#8a7a55;margin-bottom:6px}
    ...
    .dh-progress-fill{height:100%;background:linear-gradient(90deg,#d4af37,#f0c040);border-radius:99px;transition:width .35s ease}
    .dh-spark-chip{display:inline-flex;align-items:center;gap:6px;background:rgba(240,192,64,.1);border:1px solid rgba(240,192,64,.35);color:#f0c040;border-radius:99px;padding:6px 12px;font-size:.85rem;margin:6px 0 10px}
    .dh-stage-hint{color:#8a7a55;font-size:.8rem;text-align:center;margin:0 0 12px}
    .option{transition:transform .15s ease,box-shadow .15s ease,border-color .15s}
    .option:active{transform:scale(.98)}
    .option.selected{transform:scale(1.01)}
    .swipe-card{animation:dhIn .28s ease}
    @keyframes dhIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:none}}
    .dh-result-hero{background:linear-gradient(145deg,#1a1a2e,#241a12);border:1px solid rgba(240,192,64,.35);border-radius:16px;padding:18px;margin:12px 0;text-align:center;box-shadow:0 8px 28px rgba(0,0,0,.35)}
    .dh-result-score{font-size:2rem;color:#f0c040;font-weight:800;letter-spacing:-1px}
    .dh-share-box{background:#12121c;border:1px dashed rgba(240,192,64,.4);border-radius:12px;padding:12px;margin-top:12px;font-size:.85rem;color:#cbb98a;line-height:1.9}
    .dh-loading{text-align:center;padding:40px 16px}
    .dh-loading .spin{width:28px;height:28px;border:3px solid #2a2a3e;border-top-color:#f0c040;border-radius:50%;margin:0 auto 14px;animation:dhSpin .8s linear infinite}
    @keyframes dhSpin{to{transform:rotate(360deg)}}
    .btn-primary{transition:transform .15s ease,box-shadow .15s}
    .btn-primary:active{transform:scale(.97)}
    .option-icon{font-size:1.7rem;line-height:1;margin-bottom:6px;display:block}
  `;
...
function sparkChipHTML() {
  ...
  const min = 20, max = 80;
  let hint = n < min ? `حداقل ${min - n} جرقهٔ دیگر` : (n >= max ? 'حداکثر رسیده' : 'ادامه بده');
...
function renderSplash() {
  ...
  const actionButtonHTML = hasSession
    ? `<button class="btn btn-primary" style="width:100%;" onclick="resumeJourney()">ادامه سفر</button>`
    : `<button class="btn btn-primary" style="width:100%;" onclick="startNewJourney()">ورود به شهر</button>`;
  ...
  app.innerHTML = progressHTML('splash') + `
    <div style="text-align:center;padding:20px 10px;">
      ...
      <div class="card" style="text-align:right;">
        <p class="quote">«جایی که هر کودکی قبل از خواب به آن سفر می‌کرد...»</p>
        <p>یادت می‌آید بچه که بودی، چشمانت را می‌بستی و خودت را جای یک نفر دیگر تصور می‌کردی؟ یک روز دکتر بودی، یک روز خلبان، یک روز نقاش، یک روز هم کاشف سیارات دور. آن تصویرها هنوز جایی در وجودت زنده‌اند.</p>
        <p>حالا وقت برگشتن است؛ این‌بار با نگاه دقیق‌تر. در شهر رؤیاها شش محله هست. هر محله بوی خودش را دارد و آدم‌هایش کاری می‌کنند که انگار برای همان کار به دنیا آمده‌اند.</p>
        <p><strong style="color:#f0c040;">کدام محله هنوز قلبت را به تپش می‌اندازد؟</strong></p>
        ${actionButtonHTML}
      </div>
...
function renderRealm() {
  ...
  let html = progressHTML('realm') + sparkChipHTML() + `<h2>🌃 شهر رؤیاها</h2>
...
function renderSubRealm() {
  ...
  let html = progressHTML('subRealm') + sparkChipHTML() + `<h2>راهروهای محله</h2>
...
async function analyzeMajors() {
  ...
  app.innerHTML = progressHTML('results') + loadingHTML('داره رشته‌های نزدیک به تو رو پیدا می‌کنه...');
  try {
...
async function analyzeBranches() {
  ...
  const payload = buildPayload();
  state.lastPayload = payload;
  app.innerHTML = progressHTML('results') + loadingHTML('داره شاخه‌های نزدیک به تو رو پیدا می‌کنه...');
  try {
...
function displayResults(data, type) {
  ...
  const valueStyle = state.valueAnswers.length >= 5 ? analyzeValueStyle(state.valueAnswers) : null;
    ...
    <p class="dh-stage-hint">بر اساس جرقه‌ها، راهبرد و ارزش‌هایی که امروز از خودت نشون دادی</p>
    <div class="dh-share-box">💛 ${state.likedCodes.length} جرقه ثبت شد · این نقطهٔ شروع کشف خودته، نه پایان راه.</div>
  ...
  const shareText = 'در سفر «شهر رؤیاها» این مسیرها به من نزدیک‌تر بودن:\n' + topNames + '\n\nجرقه‌ها: ' + state.likedCodes.length + '\nاسب سیاه — کشف مسیر بر اساس فردیت';
