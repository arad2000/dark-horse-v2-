...
function renderSplash() {
  ...
  const actionButtonHTML = hasSession
    ...
    : `<button class="btn btn-primary" style="width:100%;font-size:1.05rem;padding:14px 28px;" onclick="startNewJourney()">ورود از دروازهٔ شهر 🌙</button>`;

  app.innerHTML = progressHTML('splash') + `
    <div class="dh-hero">
      <div class="dh-stars">· · · ✦ · · ·</div>
      <div class="dh-gate">دروازهٔ ششمین شب</div>
      <h1 class="dh-hero-title">شهر رؤیاها</h1>
      <p class="dh-hero-sub">جایی که مسیر، از داخل تو شروع می‌شود</p>
    </div>
    <div class="card dh-quote-card" style="text-align:right;">
      <p class="quote">قبل از آنکه کسی به تو بگوید چه باید بشوی، خودت جایی در خیال بودی.</p>
      <p>یادت هست چشمانت را می‌بستی و یک روز پزشک بودی، یک روز خلبان، یک روز معمار خاموش یک دنیای دیگر؟</p>
      <p>آن تصویرها از بین نرفتند. فقط زیر گرد و خاک «باید»ها و رتبه‌ها گم شدند.</p>
      <p style="color:#f0c040;font-weight:600;margin-top:10px;">امشب دوباره درِ شهر باز است. شش محله منتظرند. فقط یکی نیست که درست باشد — آن محله‌ای درست است که وقتی اسمش را می‌شنوی، یک‌جور آرام در سینه جابه‌جا می‌شود.</p>
      ${actionButtonHTML}
    </div>
    <button class="btn" style="margin-top:12px;width:100%;" onclick="goTo('guide')">بازگشت به راهنما</button>`;
}
