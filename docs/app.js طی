...
function progressHTML(stageId) {
  ...
  if (idx < 0) idx = 0;
  // collapse duplicate labels for percent
  ...
  return `
    <div class="dh-progress-wrap">
      <div class="dh-progress-labels">
        <span>سفر اکتشافی</span>
        <span class="on">${label} · ${pct}%</span>
      </div>
...
function sparkChipHTML() {
  ...
  const min = 20, max = 80;
  let hint = n < min ? `${min - n} جرقه تا تصویر اول` : (n >= max ? 'آتش‌دان کامل است' : 'داری روشن‌تر می‌شوی');
