...
def normalize(value: Any) -> Any:
    if isinstance(value, dict):
        return {str(k): normalize(value[k]) for k in sorted(value, key=lambda x: str(x)) if str(k) not in NON_SEMANTIC_BRANCH_FIELDS}
...
def materialize_engine_from_db(db: Session) -> DarkHorseEngineV2:
    ...
    for major_id, motive_id in db.execute(select(major_micro_motives.c.major_id, major_micro_motives.c.motive_id)).all():
    ...
    for row in db.scalars(select(Major)):
        majors[int(row.id)] = {"id": int(row.id), "name": row.name, "group": row.group, "cluster": row.cluster, "subgroup": row.subgroup, "exam_group": row.exam_group, "high_school_branch": row.high_school_branch, "strategy_weights": row.strategy_weights, "value_weights": row.value_weights, "archetype": row.archetype, "fulfillment_source": row.fulfillment_source, "prestige_level": row.prestige_level, "handcrafted": row.handcrafted, "motive_driven": row.motive_driven, "weights_version": row.weights_version, "micro_motive_codes": major_links.get(int(row.id), [])}
    branch_links: dict[int, list[str]] = {}
    for branch_id, motive_id in db.execute(select(branch_micro_motives.c.branch_id, branch_micro_motives.c.motive_id)).all():
    ...
    for row in db.scalars(select(SchoolBranch)):
        branches.append({"id": row.id, "name": row.name, "group": row.group, "m_score_denom_limit": row.m_score_denom_limit, "strategy_weights": row.strategy_weights, "value_weights": row.value_weights, "weights_version": row.weights_version, "source_majors_count": row.source_majors_count, "micro_motive_codes": branch_links.get(int(row.id), [])})
...
def main() -> None:
    ...
    json_engine = DarkHorseEngineV2(motives_path=str(ROOT / "docs/data/micro_motives.json"), majors_path=str(ROOT / "majors_database_v2.json"), trait_map_path=str(ROOT / "docs/data/trait_map_v3.json"), value_poles_path=str(ROOT / "value_poles_v2.json"), school_branches_path=str(ROOT / "school_branches_v2.json"))
    ...
    for fixture in fixtures:
        ...
        reports.append({"name":fixture["name"],"major_equal":normalize(jm)==normalize(pm),"branch_equal":normalize(jb)==normalize(pb),"json_best_branch":jb.get("best_branch"),"postgres_best_branch":pb.get("best_branch")})
    status="PASS" if all(r["major_equal"] and r["branch_equal"] for r in reports) else "FAIL"
    print(json.dumps({"status":status,"runtime_cutover":"OFF","semantic_comparison":True,"ignored_branch_metadata":sorted(NON_SEMANTIC_BRANCH_FIELDS),"fixtures":reports,"contract":{"json_engine_is_live_source":True,"postgres_engine_used_only_for_staging_dual_run":True,"comparison_scope":["recommendations","ordering","scores","components","evidence","warnings","best_branch","alternative_paths"]}},ensure_ascii=False,indent=2))
