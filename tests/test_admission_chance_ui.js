...
assert.match(source, /quota_type/);
...
assert.match(source, /رتبه در سهمیه \(کارنامه ملاک عمل انتخاب رشته سنجش\)/);
assert.match(source, /معدل کتبی نهایی دیپلم/);
assert.match(source, /DHAdmissionChanceUI/);
assert.doesNotMatch(source, /region_zone/);
assert.doesNotMatch(source, /form\.quota/);
assert.doesNotMatch(source, /darkhorse\/discover/);
assert.doesNotMatch(source, /individuality_fit\s*=|individuality_fit\s*=/);

const provinceCount = (source.match(/'[^']+'(?=,|\n\s*\])/g) || []).length;
assert.ok(source.includes('همدان'));
assert.ok(source.includes('هرمزگان'));
...
assert.match(indexSource, /admission_chance_ui\.css\?v=2/);
assert.match(indexSource, /admission_chance_ui\.js\?v=2/);

console.log('admission_chance_ui phase2 regression: PASS');
