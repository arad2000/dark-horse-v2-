...
class CommercialBillingE2ETests(unittest.TestCase):
    ...
    def test_register_consume_buy_callback_replay_consume(self):
        ...
        self.assertNotIn("id", register.json()["user"])
        ...
        self.assertNotIn("entitlement_id", consumed_free.json())
        ...
        self.assertNotIn("payment_id", purchase_body)
        self.assertIn(f"order_id={purchase_body['order_id']}", purchase_body["callback_url"])
        ...
        self.assertNotIn("user_id", quota.json())
        ...
        blocked = self.client.get("/api/v1/me/quota", headers=headers)
